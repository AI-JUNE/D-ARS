...
- [ ] **약관·개인정보 처리방침 확정본 반영** (현재 초안, 문안은 사람이 확정)
  - **미완인 이유**: 문안 확정·법무 검토·사업자 정보는 사람의 일이고 자동화가 대신할 수 없다. 이 항목은
    사람의 차례가 올 때까지 `[ ]` 로 남는다. 자동화가 지어낸 사업자 정보·수탁자 목록은 넣지 않았다.
  - 대신 넣은 것(확정 게이트): 지금 구조에서는 `LEGAL_META.status` 를 `'published'` 로 **한 글자만 바꾸면**
    화면의 초안 배너가 사라지고, 사업자등록번호·수탁자 목록·보유기간이 빈 문서가 그대로 정식 고지가 된다.
    오류도 경고도 나지 않는다. 그래서 확정 조건을 체크리스트가 아니라 **판정**으로 옮겼다 —
    `lib/legalReadiness.js` + `npm run legal:check`(읽기 전용 · 네트워크·DB 무접속 · 환경변수 값 미출력) + CI 단계.
    초안 단계는 통과(빈 칸은 아직 사람의 차례)하되 남은 칸을 매번 출력하고, `published` 로 넘기는 순간
    빈 칸·미확정 표식이 하나라도 있으면 실패한다. 판정 코드 8종(`META_MISSING`·`PENDING_MARKERS`·
    `SUBPROCESSORS_EMPTY`·`CLAIM_AHEAD_OF_CODE`·`RETENTION_VAGUE`·`DRAFT_UNMARKED`·형식 2종).
  - 드러낸 사실(고치지 않고 **정직하게 표시**): ① 방침 7장은 "접근권한 관리(RBAC)·접근 통제를 시행합니다" 라고
    단언하는데 라이브는 `AUTH_ENFORCE` 가 꺼져 있다 — 초안 단계라 지금은 경고지만, 확정본에서 이 어긋남은
    **허위 서술**이라 차단으로 승격된다(실인증을 켜거나 **[승인 필요]**, 문안을 현재 조치만 서술하도록 고치거나).
    ② 처리위탁 수탁자 공개 목록(개인정보 보호법 §26)이 비어 있어 방침 5장이 "위탁할 수 있다" 일반론에 머문다.
    ③ 보유기간이 "지체 없이 파기" 일반론뿐이다. ④ 공식 문의 주소가 개인 메일 도메인이다(경고 아님 · 권고).
  - 정본에 **빈 칸을 명시적으로 선언**했다 — `businessNumber`·`address`·`privacyOfficer`·`retentionNotice`·
    `SUBPROCESSORS`. 비어 있는 것이 정상이고, 임의로 채워지지 않았음을 테스트가 감시한다.
  - 채울 항목·출처·게시 절차는 `docs/LEGAL_READINESS.md`. 문안 확정과 `status` 전환은 **[승인 필요]**.
  - 검증: `tests/legalreadiness.test.mjs` 15케이스(초안/게시본 판정·필수 항목 8종 개별 지목·오타 내성·
    보유기간 오탐 방지·환경변수 값 미유출·CLI 읽기 전용 가드·CI 배선 대조) · 기존 `tests/legal.test.mjs` 12케이스 유지.
  - 한계: 이 게이트는 **문안의 법적 적정성을 판정하지 않는다**(칸이 채워졌는지, 서술이 코드 상태와 어긋나지
    않는지만 본다 — 법무 검토를 대체하지 않는다). `CLAIM_AHEAD_OF_CODE` 는 접근통제 서술 하나만 대조한다.
    개정 이력(버전 누적)은 관리하지 않는다.
...
- [x] 세션 토큰 보안 요건 점검 — 만료·1회용·엔트로피
  ...
  - 정정(이번 회차) — 위 ①은 **더 이상 사실이 아닌데도 계속 그렇게 보고되고 있었다**. `lib/eumConsume`
    로 1회용 소진이 구현된 뒤에도 `TOKEN_SPECS.eum-link.oneTimeImplemented` 가 `false` 로 남아,
    `token:check` 가 매번 "요건은 1회용이지만 실제로는 재사용 가능한 베어러" 라고 **거짓 보고**했다.
    앞서 걱정한 것은 서술이 코드보다 **앞서는** 경우였는데, 이번엔 반대로 **뒤처져** 이미 한 일을 안 했다고
    말하고 있었다 — 어느 쪽이든 판정을 믿을 수 없게 만드는 것은 같다. 불리언으로는 지금 상태를 말할 수
    없어(구현됐으나 기록이 인스턴스 로컬이라 절대 보장은 아니다) 세 수준 `oneTime: none|local|durable` 로
    바꾸고, `local` 전용 경고 `TOKEN_ONE_TIME_LOCAL`(공유 저장소 영속화 **[승인 필요]**)을 새로 냈다.
    재발 방지: 선언한 수준을 **실제 소진 배선**(라우트의 `consumeStore().claim(`)과 저장 방식(`new Map()`
    이면 local · 주석은 제외하고 실행되는 줄만 본다)에 대조해, 앞뒤 어느 쪽으로 어긋나도 테스트가 실패한다.
    `docs/TOKEN_SECURITY.md` 3장·표·6장 갱신. 검증: `tests/tokenaudit.test.mjs` 25→29케이스.
- [x] 장애 폴백 — 엔진 오류 시 기존 IVR 전환 경로 검증
  ...
  - 보완(이번 회차) — 위 폴백에 **구멍이 하나 남아 있었다**: 고객 화면 폴링이 상한 없는 맨 `fetch` 라,
    연결은 살아 있는데 서버가 응답하지 않으면(모바일 음영·콜드스타트) 요청이 성공도 실패도 하지 않아
    **연속 실패 카운터가 올라가지 않고 끊김 안내가 영영 뜨지 않았다**. 즉 이 항목이 막는다고 적어 둔 바로 그
    사고 — "콜봇이 화면을 보라고 안내한 뒤 고객이 멈춘 화면 앞에 남는 것" — 가 그대로 가능했다. 세는 규칙은
    있었지만 **응답 없음이 실패로 세어지지 않았다.** `lib/fetchJson.fetchOnce`(시간 상한 · 절대 throw 없음 ·
    timeout/offline/network 구분)로 바꾸고, 주기·상한·실패 기준을 `lib/ivrFallback` 상수 단일 출처로 모았다
    (상한 2초 < 주기 2.5초 — 멈춘 요청이 다음 폴링 전에 실패로 확정되고 겹겹이 쌓이지 않는다).
    같은 결함이 화면 전반에 있어 함께 고쳤다 — 「이음 어르신 신청」 제출(단추가 disabled 인 채 "신청하는 중…"
    에서 멈추고 그 사이 5분 만료로 링크까지 죽었다) · 로그인(비JSON 오류 응답을 "네트워크 오류" 라 잘못
    안내하던 것 포함) · 로그아웃 · `/api/auth/me`·`/api/health` 조회. 이제 `app/**/*.jsx` 에 상한 없는
    맨 `fetch` 가 있으면 테스트가 실패한다(현재 0건 · 예외 목록 없음).
  - 검증: `tests/nohang.test.mjs` 11케이스(상한<주기 불변식 · 응답 없는 서버에서 폴백이 실제로 켜지는지 ·
    상한 없을 때 폴링이 끝나지 않음을 재현 · 화면 전역 스캔 · 실패 안내 문구) + `tests/fetchonce.test.mjs` 14케이스
    (timeout/offline/network 구분 · 4xx·5xx 를 실패로 뭉개지 않음 · 이상 입력 무throw · 안내 문구에 기술용어 미노출)
...
- [x] **파트너(채널) 개념 도입** — 조직/계약에 `partner_id`(nullable) 추가. 없으면 직접 계약. 스키마만 준비하고 화면 노출은 최소
  - 근거: 저장소에 조직·계약 표가 **아예 없었다**(테넌트 개념 부재). `db/partner.sql` 에 `partners` · `organizations`(`partner_id` nullable → NULL=직접 계약, `acquired_via` 와의 정합을 check 제약으로 고정) 를 추가. 화면·API 배선 0, 환경변수 0. `npm run db:setup` 은 이 파일을 적용하지 않는다(테스트가 고정) — 운영 적용은 **[승인 필요]**
  - 등록부 갱신: `lib/backupCheck.COVERED_TABLES`·RUNBOOK 자산표·복구 순서, `lib/retentionScan.PII_EXEMPT`(법인명 2·운영자 표시명 1) + `docs/PRIVACY_RETENTION.md` — 기존 양방향 대조 테스트가 모두 통과하도록 같은 회차에 맞췄다. 문서 `docs/PARTNER_CHANNEL.md`
  - 검증: `tests/partner.test.mjs` 16케이스(식별자·날짜·채널 유도·정합 위반·이상 입력 무throw·스키마 check 제약 ↔ 코드 규칙 대조·PII 컬럼 부재·수수료율 하드코딩 금지·db-setup 미적용)
  - 한계: 2계층(리셀러)·화이트라벨은 미구현 — 테넌트 조회에 끼워 넣을 단일 지점(`scopeOrganizations`/`scopeSql`)만 열어 뒀다. 정산·청구는 계약서 확정 후
...
- [x] **2계층 확장 여지 확보** — 테넌트 조회 경로에 파트너 필터가 나중에 끼어들 수 있도록 쿼리 계층 정리. 지금 화이트라벨은 구현하지 않음
  - 근거: 여태 확보한 것은 **판정 함수**(`partnerScopeOf`·`scopeSql`)뿐이었고 실제 조회는 라우트마다 손으로 SQL 을
    적는 구조였다(정산 라우트의 `sql\`select … from organizations\``). 이 구조의 결함은 하나다 — **범위 필터를 빠뜨려도
    아무 일도 일어나지 않는다.** 빌드·테스트·화면 모두 멀쩡하고, 리셀러 전환으로 파트너 계정이 실제로 켜지는 순간
    한꺼번에 드러난다. 그래서 "끼워 넣을 수 있게" 가 아니라 **"범위를 말하지 않으면 질의가 만들어지지 않게"** 설계했다.
  - `lib/tenantQuery.js` — 테넌트 표 3종(`partners`·`organizations`·`partner_attributions`) 조회의 단일 지점.
    불변식 넷: (1) `scope` **필수 인자** — 빠뜨리면 전체가 아니라 빈 결과(`SCOPE_REQUIRED` + `where false`).
    실수의 기본값이 '전체 공개' 가 아니라 '아무것도 안 보임' 이다. (2) 표·컬럼·정렬키는 등록부 화이트리스트에서만 —
    모르는 표는 이름조차 질의에 넣지 않는다. (3) 값은 전부 `$1..` 바인딩(문자열 결합 경로 없음). (4) select 전용.
    범위 규칙은 표마다 다르다 — `organizations` 직접(`partner_id`), `partners` 자기 행, `partner_attributions` 는
    고객사를 거친 하위 질의.
  - `lib/auth.viewerScope(req)` — 요청자의 범위를 한 줄로. 비강제(데모) 모드에 신원이 없으면 `null`(전체 —
    그 모드는 인증 자체가 없어 범위 제한이 성립하지 않는다 · `guardWrite` 와 같은 계약), 강제 모드에 신원이 없으면
    빈 결과로 닫는다.
  - 이관: `/api/admin/settlement` 의 직접 조회 3건을 `selectTenantRows` 로 교체(컬럼·정렬·폴백 동일 → **동작 불변**).
    admin 게이트를 통과한 사람은 정의상 직원이지만 범위는 그래도 요청자에게서 받아 온다 — 게이트 구성이 바뀌어도
    조회가 알아서 좁아지도록.
  - 우회 금지: `app/**`·`lib/**` 전 소스를 스캔해 테넌트 표를 직접 조회하는 파일이 있으면 실패한다(예외는 사유 필수 ·
    유령·낡은 예외도 실패). 등록부 ↔ `db/partner.sql` 양방향 대조로 표·컬럼이 늘거나 줄면 등록부 갱신을 강제한다.
  - 검증: `tests/tenantquery.test.mjs` 21케이스(범위 6·화이트리스트 5·실행 헬퍼 3·우회 스캔 4·스키마 대조 3) ·
    전체 `node --test "tests/*.test.mjs"` **969/969 통과** · coverage 게이트 로직 61/61 · env/auth/backup/retention CLI 이상 없음.
  - 한계(정직하게): 이 계층을 쓰는 라우트는 **아직 정산 리포트 하나뿐**이다(테넌트 표를 읽는 유일한 라우트).
    `visual_sessions` 등 운영 데이터는 고객사와 연결돼 있지 않아 범위 대상이 아니며, 연결되는 시점에 등록부 한 줄을
    더하는 것이 남은 일이다. 화이트라벨·파트너 명의 계약(리셀러)은 여전히 **미구현**이고, `PARTNER_ROLE_ENABLE` 전환은
    **[승인 필요]** 그대로다. 우회 스캔은 정적 문자열 기준이라 동적으로 조립한 표 이름은 잡지 못한다.
