// CI 전용 — AICC-Core 채널 계약 적합성 검사에 넘기는 D-ARS 포트.
//
// **드라이런 전용**이다. 전송기(sink)를 주입하지 않으므로 고객 화면으로 아무것도 나가지 않는다.
// 실푸시는 런타임 진입점에서 sink 를 주입할 때만 일어나며 [승인 필요] 다.
//
