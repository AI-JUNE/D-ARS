...
export const ENV_VARS = [
  ...
  { name: 'PARTNER_ROLE_ENABLE', scope: 'runtime', secret: false, gate: true,
    fallbackKo: 'OFF(파트너 계정 로그인·권한 판정 거부)',
    profiles: { demo: EXPECT_UNSET, staging: EXPECT_UNSET, production: EXPECT_UNSET },
    note: 'partner_admin 역할 활성화. 테넌트 범위 배선 확인 후 사람이 켠다' },
  { name: 'PARTNER_COMMISSION_RATES', scope: 'runtime', secret: false, gate: false,
    fallbackKo: '수수료 미산출(rate_missing · 0 으로 꾸미지 않음)',
    profiles: { demo: EXPECT_UNSET, staging: EXPECT_ANY, production: EXPECT_ANY },
    note: '파트너 수수료율 JSON(퍼센트 · 키=파트너 id 또는 *). 계약서 확정값만 넣는다(lib/settlement.js)' },
