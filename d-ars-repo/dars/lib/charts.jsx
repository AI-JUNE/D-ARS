...
import { fmtMD, finiteNums, meterLabel, trendLabel, barsLabel } from '@/lib/ui';
import { fmtNum } from '@/lib/kpi';   // 툴팁 수치 ko-KR 고정·비유한값(문자열/undefined/NaN) 0 방어 — 앱 전역 숫자 표기 통일(fmtNum)
...
export function AreaChart({ data = [], labels = [], color = '#be5535', height = 176, unit = '', mini = false }) {
  ...
  // 비유한 요소(null/undefined/NaN 등)를 0으로 방어 정규화 → 단 하나라도 섞이면 max/min·Y(v)=NaN 으로
  // SVG path 전체가 조용히 깨지던 문제를 차단(유효 숫자는 항등 → 출력 100% 불변). GroupedBars 와 동일 패턴.
  ...
  let len = 0;
  for (let i = 1; i < n; i++) len += Math.hypot(X(i) - X(i - 1), Y(D[i]) - Y(D[i - 1]));
  ...
  return (
    <div className="ac-wrap" style={{ position: 'relative' }} onMouseMove={onMove} onMouseLeave={() => setIdx(-1)}>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={mini ? height : 'auto'} preserveAspectRatio="none" style={{ display: 'block', overflow: 'visible' }} role="img" aria-label={trendLabel(D, unit)}>
        <defs>
      ...
      {!mini && idx >= 0 && (
        <div className="ac-tip" style={{ left: `${(X(idx) / W) * 100}%`, top: `${(Y(D[idx]) / H) * 100}%` }}>
          <b>{fmtNum(D[idx])}{unit}</b>
    ...
    <div style={{ position: 'relative' }}>
      ...
      {hi >= 0 && (
        <div className="ac-tip gb-tip" style={{ left: `${((padX + gw * hi + gw / 2) / W) * 100}%`, top: 6 }}>
          <b style={{ fontSize: 11 }}>{fmtMD(data[hi][labelKey])}</b>
          {series.map(s => (
            <span key={s.key} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
              ...
              {s.label} <b style={{ marginLeft: 'auto' }}>{fmtNum(data[hi][s.key])}</b>
            </span>
          ))}
        </div>
      )}
